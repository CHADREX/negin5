...
function jcaa_get_product_attributes($attributes){
	...
	foreach( $attributes as $rawName => $data){
		...
		if( ($jcaa_attribute_grouped === 'yes' || !$data->is_taxonomy()) && $data->get_visible() == 1 ){
			...
			remove_filter( 'woocommerce_attribute_label', 'jcaa_attribute_label', 10, 3);

			// used get the correct translation
			$label = __(wc_attribute_label($data->get_name()), 'jcaa');

			// re-add filter
			add_filter( 'woocommerce_attribute_label', 'jcaa_attribute_label', 10, 3);
...
function jcaa_attribute_label($label, $name , $product){
	...
	global $wpdb;
	$id = $wpdb->get_var( $wpdb->prepare( "SELECT attribute_id FROM {$wpdb->prefix}woocommerce_attribute_taxonomies WHERE attribute_label = %s;", $label ) );
...
//		$term_id = $translated_term_id > 0 ? $translated_term_id : $term->term_id;
		...
		$option['img'] =  wp_get_attachment_thumb_url( get_term_meta( $term_id, '_jcaa_product_attr_thumbnail_id', true ) );
		$option['color'] = get_term_meta( $term_id, '_jcaa_product_attr_color', true );
