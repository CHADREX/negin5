...
add_action( 'woocommerce_product_thumbnails', 'tm_woocompare_add_button_single', 35 );
...
function tm_woocompare_add_button( $args ) {
	...
	if ( in_array( $id, tm_woocompare_get_list() ) ) {
		...
		$text      = 'این محصول در لیست مقایسه وجود دارد.';
		$classes[] = ' in_compare';
	...
	$text      = '<span class="tm_woocompare_product_actions_tip">' .$text. '</span>';
...
function tm_woocompare_page_button() {
	...
	$text    = '<i class="far fa-clone"></i> لیست مقایسه';
