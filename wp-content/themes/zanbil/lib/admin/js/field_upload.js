(function($) {
	$(document).ready(function(){
		...
		$("img[src='']").attr("src", zanbil_menu_upload.url);
		
		$('.zanbil-menu-upload').on('click', function() {
			header_clicked = true;
		...
		$('.zanbil-menu-upload-remove').on('click', function(){
			$relid = $(this).attr('rel-id');
			$('#'+$relid).val('');
			$(this).prev().fadeIn('fast');
			$(this).prev().prev().fadeOut('fast', function(){$(this).attr("src", zanbil_menu_upload.url);});
			$(this).fadeOut('slow');
		});
	});
	function MenuClick(){
