 ...
 */
function zanbil_customize_register( $wp_customize ){

	$priority = 200;
	$sections = array();
	$zanbil_options = zanbil_options();	
	$i = 0;
	foreach ( $zanbil_options->sections as $section) {
		...
		if ( isset($section['fields']) && is_array($section['fields']) ) {
			...
			foreach ($section['fields'] as $field ){
				...
				if ($zanbil_options->get($customize)) {
					...
					$field['settings'] = $zanbil_options->args['opt_name'].'['.$field['id'].']';
	...
	foreach ( $sections as $section ) {
		...
		foreach ($section['fields'] as $field ){ 
			...
			$wp_customize->add_setting( 
					$zanbil_options->args['opt_name'].'['.$field['id'].']', 
					array(
						'default'  	=> $zanbil_options->get($field['id']),
    					'type' 		=> 'option',
...
add_action( 'customize_register', 'zanbil_customize_register' );
